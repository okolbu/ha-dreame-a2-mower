...
class DreameMowerMapDecoder:
    """Upstream binary-blob map decoder — stubbed 2026-04-20 (Phase 4).

    The A2 (dreame.mower.g2408) gets its map from the Dreame cloud's
    MAP.* JSON (see device.py::_build_map_from_cloud_data), not from
    binary map blobs pushed over MQTT. The ~1380-line decoder that
    lived here was permanent dead code for every A2 user.

    All public methods are retained as no-op stubs so the map manager
    / editor / optimizer call sites that still reference them keep
    compiling without crashing. If a Dreame firmware update ever
    starts pushing binary maps to the A2, the stubs will silently
    drop them — less surprising than a NameError — and a `[MAP_STUB]`
    warning will fire on first contact so a future contributor has
    a clear signal to revert or re-implement.
    """

    HEADER_SIZE = 27

    @staticmethod
    def decode_map_partial(raw_map, iv=None, key=None):
        if raw_map:
            logging.getLogger(__name__).warning(
                "[MAP_STUB] decode_map_partial called with %d-byte payload; "
                "A2 does not support binary map blobs — dropping",
                len(raw_map) if hasattr(raw_map, "__len__") else -1,
            )
        return None

    @staticmethod
    def decode_p_map_data_from_partial(*args, **kwargs):
        return None

    @staticmethod
    def decode_map_data_from_partial(partial_map, vslam_map=False):
        return (None, None)

    @staticmethod
    def decode_map(raw_map, vslam_map=False, rotation=0, iv=None):
        return (None, None)

    @staticmethod
    def decode_saved_map(raw_map, vslam_map=False, rotation=0, iv=None):
        return None

    @staticmethod
    def decode_cleaning_map_data(*args, **kwargs):
        return None

    @staticmethod
    def set_segment_cleanset(map_data, cleanset, capability):
        return None
    ...
    @staticmethod
    def set_robot_segment(map_data):
        return None

    @staticmethod
    def get_segments(map_data, vslam_map=False):
        return {}
