"""Live-map render orchestration service (layer 4) — refactor-v2 P3.9e.

Moved VERBATIM from ``coordinator/_rendering.py`` (the ``_RenderingMixin``).
Each function takes the coordinator (``coord``) as its first argument; the
coordinator keeps thin ``_RenderingMixin`` delegators so the public/test
surface (``coord._render_base`` / ``coord._render_active_map_base`` /
...
unbound ``_RenderingMixin._X`` methods bound by
``test_live_stream_publish`` / ``test_base_render_on_activity`` /
``test_obstacle_live_render``, and the ``coordinator._rendering``
``LIVE_TRACK_SNAPSHOT_MAX`` re-export) is unchanged.

This is presentation-orchestration: it reaches UP into ``map_render`` (layer 6)
via function-local imports only — the same accepted pattern the layer gate
explicitly excludes (``tests/audit/test_layer_imports.py`` docstring names
``coordinator/_rendering.py`` as the exemplar). Internal cross-method calls
route through the coordinator delegators (``coord._compute_background_mode`` /
``coord._live_obstacle_polygons`` / ``coord._load_last_session_obstacles`` /
``coord._render_active_map_base`` / ``coord._render_base``) so test
monkeypatches / partial method-binds still intercept.
"""
...
def current_mower_position(coord) -> tuple[float, float] | None:
    ...
    x = coord.data.position_x_m
    y = coord.data.position_y_m
    if x is None or y is None:
        return None
    return (float(x), float(y))


def compute_background_mode(coord) -> "BackgroundMode":
    ...
    return background_mode_for(
        ...
        action_mode=getattr(coord.data, "action_mode", None),
    )


def schedule_render_base(coord) -> None:
    ...
    hass.loop.call_soon_threadsafe(
        lambda: hass.async_create_task(coord._render_base())
    )


def live_obstacle_polygons(coord) -> "list[list[tuple[float, float]]]":
    """Current live AIOBS markers as render-ready polygons (metres, ≥3 pts)."""
    out = []
    for m in coord._obstacle_markers or []:
...
async def render_base(coord) -> None:
    ...
    if active_id is None:
        return
    map_data = coord.cloud_state.maps_by_id.get(active_id)
    if map_data is None:
        return
    mode = coord._compute_background_mode()
    md5 = getattr(map_data, "md5", None)
    from ..map_render import BackgroundMode
    live = coord._live_obstacle_polygons()
    if mode == BackgroundMode.GREEN:
        obstacles = live or None
    else:
        obstacles = await coord._load_last_session_obstacles(active_id)
    ...
    direction = getattr(coord.data, "settings_mowing_direction", None)
    if (
        coord._base_png is not None
        and coord._base_png_mode == mode
        and coord._base_png_md5 == md5
        and coord._base_png_marker_fp == marker_fp
        and coord._base_png_direction == direction
    ):
        return  # fresh render already cached
    from functools import partial
    from ..map_render import render_base as render_base_png
    png = await coord.hass.async_add_executor_job(
        partial(
            render_base_png, map_data,
            background_mode=mode, state=coord.data,
            map_id=active_id, obstacle_polygons_m=obstacles,
        )
    )
    if png:
        coord._base_png = png
        coord._base_png_mode = mode
        coord._base_png_md5 = md5
        coord._base_png_marker_fp = marker_fp
        coord._base_png_direction = direction
        LOGGER.debug(
        ...
        editor_png = await coord.hass.async_add_executor_job(
            partial(
                render_base_png, clean_md,
                background_mode=mode, state=coord.data,
                map_id=active_id, obstacle_polygons_m=obstacles,
            )
        )
        if editor_png:
            coord._editor_base_png = editor_png
    # Keep the Work Log camera's empty-state CLEAN base fresh too.
    await coord._render_active_map_base()


async def render_active_map_base(coord) -> None:
    ...
    if (
        coord._active_map_base_png is not None
        and coord._active_map_base_md5 == current_md5
    ):
        return
    from ..map_render import render_base_map
    png = await coord.hass.async_add_executor_job(render_base_map, map_data)
    if png:
        coord._active_map_base_png = png
        coord._active_map_base_md5 = current_md5


def begin_live_stream(coord) -> None:
...
def publish_live_point(
    coord, *, x_m: float, y_m: float, heading_deg: float | None, t: float
) -> None:
    ...
    coord._live_point_seq += 1
    coord._latest_point = pt
    LOGGER.debug(
        "[MAP] publish: seq=%d pt=(%.2f,%.2f) hdg=%s",
        coord._live_point_seq, x_m, y_m,
        f"{heading_deg:.1f}(byte)" if heading_deg is not None else "none(vector)",
    )


def live_track_snapshot(coord) -> list[list]:
    ...
    """
    track = coord.live_map.track
    n = len(track)
    cache = coord._track_snapshot_cache
    ...
    coord._track_snapshot_cache = (n, rows)
    return rows


async def load_last_session_obstacles(
    coord, map_id: int
    ...
    """
    cached = coord._last_session_obstacles_by_map.get(map_id)
    ...
    if not candidates:
        ...
        coord._last_session_obstacles_by_map[map_id] = []
        return None
    entry = max(candidates, key=lambda s: s.end_ts)

    raw_dict = await coord.hass.async_add_executor_job(archive.load, entry)
    if raw_dict is None:
        LOGGER.info(
    ...
    coord._last_session_obstacles_by_map[map_id] = polygons
    if not polygons:
        LOGGER.info(
