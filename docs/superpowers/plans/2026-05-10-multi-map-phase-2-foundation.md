...
def test_sn_captured_from_device_info():
    ...
    assert proto._sn == "G2408000TESTSN0000"
...
def _coord(sn="G2408000TESTSN0000", mac="ef:ce:cc:aa:fe:fd",
...
def test_mower_identifiers_uses_sn():
    assert mower_identifiers(_coord()) == {(DOMAIN, "G2408000TESTSN0000")}
...
def test_mower_unique_id():
    assert mower_unique_id(_coord(), "battery") == "G2408000TESTSN0000_battery"


def test_map_unique_id():
    assert (
        map_unique_id(_coord(), 1, "lidar_top_down")
        == "G2408000TESTSN0000_map_1_lidar_top_down"
    )


def test_mower_device_info_shape():
    info = mower_device_info(_coord())
    assert info["identifiers"] == {(DOMAIN, "G2408000TESTSN0000")}
    assert info["manufacturer"] == "Dreame"
    assert info["model"] == "dreame.mower.g2408"
    assert info["serial_number"] == "G2408000TESTSN0000"


def test_map_device_info_shape():
    info = map_device_info(_coord(), 0, name="Front Lawn")
    assert info["identifiers"] == {(DOMAIN, "G2408000TESTSN0000_map_0")}
    assert info["via_device"] == (DOMAIN, "G2408000TESTSN0000")
...
async def test_sync_creates_subdevice_per_map_id(coordinator_with_two_maps):
    ...
    assert {(DOMAIN, "G2408000TESTSN0000_map_0")} in identifiers
    assert {(DOMAIN, "G2408000TESTSN0000_map_1")} in identifiers
...
async def test_sync_removes_subdevice_for_dropped_map(
    ...
    with patch.object(coord, "_get_device_registry") as mock_reg:
        ...
        existing = MagicMock()
        existing.identifiers = {(DOMAIN, "G2408000TESTSN0000_map_1")}
...
def coordinator_with_two_maps():
    ...
    coord._cloud = MagicMock(sn="G2408000TESTSN0000", mac="ef:ce:cc:aa:fe:fd",
                              model="dreame.mower.g2408")
...
async def test_migration_rewrites_battery_unique_id():
    ...
    entry.runtime_data.sn = "G2408000TESTSN0000"
    rewrites = _collect_rewrites(hass, entry)

    assert rewrites.get("abc123_battery") == "G2408000TESTSN0000_battery"
    assert rewrites.get("abc123_signal") == "G2408000TESTSN0000_signal"
```
...
async def test_zone_select_per_map(coordinator_with_two_maps):
    ...
    assert e0.unique_id == "G2408000TESTSN0000_map_0_zone_select"
    assert e1.unique_id == "G2408000TESTSN0000_map_1_zone_select"
    assert e0.device_info["identifiers"] == {
        (DOMAIN, "G2408000TESTSN0000_map_0")
    }
    assert e1.device_info["identifiers"] == {
        (DOMAIN, "G2408000TESTSN0000_map_1")
    }
...
def test_setting_switches_per_map(coordinator_with_two_maps, cls, key):
    ...
    assert e0.unique_id == f"G2408000TESTSN0000_map_0_{key}"
    assert e1.unique_id == f"G2408000TESTSN0000_map_1_{key}"
    assert e0.device_info["identifiers"] == {
        (DOMAIN, "G2408000TESTSN0000_map_0")
    }
...
async def test_per_map_snapshot_on_subdevice(coordinator_with_two_maps):
    ...
    assert cam0.unique_id == "G2408000TESTSN0000_map_0_map"
...
async def test_active_map_select_unique_id_uses_sn(coordinator_with_two_maps):
    ...
    assert e.unique_id == "G2408000TESTSN0000_active_map"
    from custom_components.dreame_a2_mower.const import DOMAIN
    assert e.device_info["identifiers"] == {(DOMAIN, "G2408000TESTSN0000")}
```
...
def test_wifi_map_camera_per_map(coordinator_with_two_maps):
    ...
    assert cam0.unique_id == "G2408000TESTSN0000_map_0_wifi_map"
    assert cam1.unique_id == "G2408000TESTSN0000_map_1_wifi_map"
    assert cam0.device_info["identifiers"] == {
        (DOMAIN, "G2408000TESTSN0000_map_0")
    }
```
...
def test_lidar_top_down_per_map(coordinator_with_two_maps, tmp_path):
    ...
    assert cam0.unique_id == "G2408000TESTSN0000_map_0_lidar_top_down"
    assert cam1.unique_id == "G2408000TESTSN0000_map_1_lidar_top_down"
    assert cam0.device_info["identifiers"] == {
        (DOMAIN, "G2408000TESTSN0000_map_0")
    }
```
...
(`G2408000TESTSN0000` here) instead of `entry_id`, so they survive
