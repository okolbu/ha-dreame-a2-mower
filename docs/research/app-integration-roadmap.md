...
| F | Map editing (zone/no-go/ignore-obstacle/rename/split/merge CRUD via routed opcodes; interactive draw card; draw-by-driving over BT deferred) | **done** (2026-06-12). Part 1: rename zone (o=219) + delete object (o=218: zone/no-go cat 0, ignore-obstacle cat 4) wired via the o=204/o=201 edit transaction + o=200 map-select; services rename_zone / delete_map_object + per-map sensor attrs (renamable_zones, deletable_objects). Part 2a: create (o=215 no-go line/poly/circle + mow-shapes 9/12-18, o=234 ignore-obstacle) + split (o=220) + merge (o=221) wired as coordinate-driven services create_no_go_zone / create_ignore_obstacle / create_mow_shape / split_zone / merge_zones (split/merge flagged destructive); coords are map edit-frame metres. Part 2b: interactive map-editor card shipped — create/edit-in-place/rotate/move/delete. Frame verified (edit-frame metres == projectPoint render frame, exact inverse; `map-edit-frame-verification-2026-06-12.md`); edit-in-place + rotate confirmed to reuse o=215 with id:<real> as the only discriminator and rotate/resize baked into points (`dreame-app-mapedit-rotate-edit-2026-06-12.md`). STILL OPEN: rename-map / delete-map (uncaptured); editing EXISTING mow-shapes (mow-shapes not decoded from the map blob, so they can't be selected/edited in the card); draw-by-driving (BT). |
| G | Live camera (Tencent XP2P P2P; off-relay, may be infeasible in HA — attempt last) | **setup chain captured** (2026-06-12). The control plane is fully wire-verified: enable `o=400 {on}` + the `dreame-third-video/tx/*` cred chain (accesstoken→isDevUser→getIdentity→getP2PInfo, SDK v2.4.49, productId B64XZHGZUT) + snapshot/record via `iotoss/addOssNew`/`ossUploaded` + retrieval via `userDidOssList`. Talk/audio ride the P2P stream (zero wire cmd). BLOCKED on consuming the media: stream is Tencent XP2P/TRTC over UDP (off-relay), needs a Python-usable XP2P SDK that may not exist — live preview deferred; pure-HTTP snapshot/record/gallery is buildable now. See `docs/research/live-video-stream-setup.md` + TODO "Live video stream + snapshot/record". |
