...
| `ATA` | `list[int × 3]` | **Anti-Theft Alarm** (confirmed 2026-04-24). Shape `[lift_alarm, offmap_alarm, realtime_location]` — matches the `s2p51` ANTI_THEFT decoder exactly. **Index [0] = Lift Alarm individually confirmed 2026-04-27** via isolated toggle (`[0,0,0] → [1,0,0]`); indices [1] and [2] order assumed-by-decoder-shape but pending individual toggle. Surfaced as `sensor.anti_theft` (state=on if any sub-flag enabled, per-flag bools in attributes). |
