...
| 6.2 | `FRAME_INFO` / **settings-saved tripwire + mow-height + mow-mode carrier** | list len 4 `[mowing_height_mm, mow_mode, True, 2]` | **Both elements decoded 2026-04-26** via live toggles. **Element [0] = Mowing Height in millimetres** — observed `70 → 60 → 50` while user stepped the app slider `7.0cm → 6.0cm → 5.0cm`. Range 30-70mm in 5mm steps (matches the app's 3-7cm in 0.5cm increments). The earlier "session-class profile id" hypothesis was wrong; values 35 / 60 / 70 in older captures were just the height in effect at the time. **Element [1] = Mowing Efficiency** — `0 = Standard`, `1 = Efficient`. Both Mowing Height and Mowing Efficiency are **NOT in CFG**: `CFG.VER` does not increment when either is toggled. Elements [2] and [3] (`True`, `2`) constant in all captures so far. **Also functions as the "settings-saved tripwire"**: every BT-only settings change kicks the device into re-publishing `s6p2` even when the value itself is unchanged. Confirmed 2026-04-22 — toggling Mowing Direction Crisscross/Chequerboard On→Off→On fired exactly two `s6p2` re-emissions per toggle (~1.5 s apart) with unchanged frame, and *no other property* moved. Surfaced as `sensor.mowing_height` (element[0] / 10) and `sensor.mow_mode` (element[1]). |
