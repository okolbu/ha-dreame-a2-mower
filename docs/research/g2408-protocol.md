...
| 6.117 | **Dock-navigation trigger** | `int` (observed: `1`) | One-shot 0→1 firing ~1 s before `s2p65 = 'TASK_NAV_DOCK'` is announced. Captured 2026-04-24 13:30:14, 3 min after a mowing_complete transition, as the mower began driving back to the dock on its own. Single observation so far; hypothesis is "firmware-side event signalling start of dock navigation" (same pattern as other siid=6 map-channel triggers). Not consumed — TASK_NAV_DOCK on s2p65 is the authoritative signal. |
