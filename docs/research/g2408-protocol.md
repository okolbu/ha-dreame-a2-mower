...
| `MSG_ALERT` | `list[int × 4]` | **Notification Preferences** — app's per-event-type push toggles. **`[0] = Anomaly Messages` confirmed 2026-04-27** via clean isolated toggle (`[1,1,1,1] → [0,1,1,1]`, VER bumped). `[1..3]` are the other three notification categories on the app's Notifications screen — index→category not yet individually mapped. Default sample is `[1,1,1,1]` (all four enabled). |
