...
| 6.117 | **Dock located / route ready** | `int` (observed: `1`) | One-shot 0→1 firing ~1 s before `s2p65 = 'TASK_NAV_DOCK'` announces and the mower starts driving home. Captured 2026-04-24 13:30:14. Key context: between mowing_complete at 13:27:18 and this push, s1p4 emissions stopped for **3 minutes and 54 seconds** — the mower was stationary somewhere on the lawn, not moving, no battery drain. Likely doing SLAM re-anchoring + dock-route planning during that pause. s6p117=1 fires when that completes, which is better described as "dock found / path computed" than "starting navigation". Single observation so far; not consumed — TASK_NAV_DOCK on s2p65 is the actionable signal. |
