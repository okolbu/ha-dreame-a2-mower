...
| `ATA` | `list[int, int, int]` | Shape matches the `s2p51` ANTI_THEFT decoder `[lift_alarm, offmap_alarm, realtime_location]` — strong candidate for Anti-Theft but pending direct toggle-confirmation. Surfaced as diagnostic `sensor.anti_theft_raw` with per-index flag attributes so a future toggle can confirm the field order. |
