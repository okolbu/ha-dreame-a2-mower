...
| `PROT` | `int {0,1}` | **Observed to flip on Navigation Path toggle (2026-04-25)** but labelling is provisional — the field name "PROT" is unexpectedly cryptic for something as user-visible as "Direct vs Smart Path", so the correlation warrants repeat testing. Second toggle test showed PROT flipping on what the user reported as a Navigation Path toggle, so the evidence is consistent; but a rigorous protocol (baseline CFG snapshot → toggle ONE setting → CFG refetch → diff → revert) across multiple settings is needed to rule out "PROT is actually some other setting whose toggle happened to coincide with the app's Navigation Path toggle in our captures". Tentatively surfaced as `sensor.navigation_path` with mapping {0=direct, 1=smart}; see `sensor.cfg_keys_raw` (alpha.116+) for the full CFG dict if the labelling turns out wrong again. Earlier alpha.89 guess "PROT = Frost Protection" already proved wrong; don't over-commit to any single interpretation. |
