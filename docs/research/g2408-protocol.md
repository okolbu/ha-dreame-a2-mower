...
| 6.2 | `FRAME_INFO` / **settings-saved tripwire + mow-mode carrier** | list len 4 `[profile_id, mow_mode, True, 2]` | **Element [1] = Mowing Efficiency** (confirmed 2026-04-26 via live toggle: Standard → Efficient flipped element[1] 0→1 in real time, no CFG key changed and `CFG.VER` did NOT increment — Mowing Efficiency is **not** in CFG at all). `mow_mode ∈ {0: Standard, 1: Efficient}`. Element [0] is a session-class profile id, A ∈ {35, 60, 70, …}, not monotonic (observed `[70,…]` 2026-04-19, `[60,…]` 2026-04-20 after Expand-Lawn / BUILDING session, `[70,…]` 2026-04-22). Elements [2] and [3] (`True`, `2`) constant in all captures. **Also functions as the "settings-saved tripwire"**: every BT-only settings change kicks the device into re-publishing `s6p2` even when the value itself is unchanged. Confirmed 2026-04-22 — toggling Mowing Direction Crisscross/Chequerboard On→Off→On fired exactly two `s6p2` re-emissions per toggle (~1.5 s apart) with unchanged frame, and *no other property* moved. Lone cloud-visible signature that "the user changed something locally"; the integration could use it to invalidate cached settings even when the changed setting itself is BT-only. Surfaced as `sensor.mow_mode` reading element[1]. |
