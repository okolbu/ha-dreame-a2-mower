...
| D17 | Pre-catalog / apk-lineage s2p2 names are current: 31/33 = "positioning-failed-stuck", 50 = "normal mow active", 71 = "positioning failure" (and ~15 other vacuum-lineage names) | S2P2_EVENT_TYPES is DERIVED from the wire-authoritative app catalog (`[apk:g2408-plugin-ext1423]`); 71 = standby-outside-station-too-long auto-return, NOT positioning-failed. Do not copy code→meaning pairs from any doc — read inventory.yaml § state_codes + mower/error_codes.py. | Debunked by: inventory.yaml § state_codes (incl. the explicit "71 is NOT positioning failed" note ~line 762); mower/error_codes.py:41-69 catalog derivation; vacuum-name cleanup commit ee2c999a; CI confidence gate (tests/inventory/test_error_codes_confidence_gate.py). Dead claim lived at docs/research/g2408-protocol.md §6 table; the doc was archived to `/data/claude/homeassistant/OLD/ha-dreame-a2-mower-docs/research/g2408-protocol.md` on 2026-07-02 (pre-P1, commit 1c1844a6) (Track-1 finding T1-1). |
| D18 | LOCN routed action is the mower GPS-position source (feeds position_lat/lon / device_tracker) | LOCN position write RETIRED; `_refresh_gps` (location/getRecords) is the sole position_lat/lon writer; s2p1 is the sole map-frame location authority. The client-side `fetch_locn` fetcher was removed in P1 (refactor-v2/p1-dead-code, 2026-07-02); the LOCN endpoint remains a documented backend fact (inventory.yaml § LOCN). | Debunked by: retirement commit 1fa47a5b + dead-code removal 5f6d7068; coordinator/_core.py:673 comment; CLAUDE.md refresher-cadence note. Dead claim's source annotation at mower/state.py:166 was fixed in P1 (refactor-v2/p1-dead-code, 2026-07-02) (Track-1 finding T1-7). |
| D19 | Direct MIoT `s6.aiid=4` requests a fresh WiFi heatmap on demand | That RPC returns 80001 on g2408 (siid-6 tunnel closed); the device generates heatmaps on its own schedule, and only already-cached OSS objects are fetchable | Debunked by: inventory.yaml § wifi-map trigger verification (evidence lines ~2510-2517, live tests 2026-05-09 + 2026-05-31). Dead belief `MowerAction.REQUEST_WIFI_MAP` was removed in P1 (refactor-v2/p1-dead-code, 2026-07-02); tombstone at mower/actions.py:320, test-only callers prior to removal (Track-1 finding T1-5). |
...
| D21 | The s2p2 "attention" tier describes a standing condition and can be latched into an "attention required" state entity (the natural counterpart to the error latch) | The attention tier is NOT a condition — it is a mixed bag of transient and SUCCESS codes. Corpus: of 180 attention-tier pushes, 97 are `human_detected` (a transient detection) and 34 are successes (`go_to_cleanpoint_success`, `cruise_task_finish`). A latch over it would sit permanently on and would latch on task COMPLETIONS. Tier is derived from (category, severity), and it is SEVERITY that separates condition from event: FAULT/consumable + ALERT/malfunction are conditions; FAULT/work_message + ALERT/anomaly are events. Tier is surfaced as attrs on `sensor.last_notification`, not latched. See inventory.yaml § s2p2 for the codes. | Debunked by: full-corpus s2p2 histogram over 15 probe logs, 742 pushes / 31 distinct codes, 2026-07-16; docs/TODO.md § s2p2 per-tier surfacing carries the per-code table. |
| D22 | The s2p50 op echo detects dock departure, merely ~42s late (so the fix is a latency fix) | Latency is the SMALLER half. The op echo is an echo of a COMMAND, so firmware/schedule-initiated mows emit none at all: it is ABSENT for 128/183 (69.9%) of real dock departures, giving the current detector recall 0.301. The ~42s median lag is itself confirmed (median 41s, n=55, reproduced across 6 logs). Re-keying is justified by COVERAGE, not earliness. | Debunked by: full-corpus dock-departure analysis over 9 evidence-carrying probe logs (183 departures), 2026-07-16; see docs/TODO.md § dock-departure repositioning UX. |
| D23 | s2p1=2 ("idle") is a LAWN state, and a 2→{6,13} event is noise | 2 is CONTEXT-DEPENDENT: idle on the lawn (`1→2→5` ×202 — that half is confirmed) AND idle ON the dock (`6→2→6` ×180 + `13→2→6` ×4). The dock-side population is charge TOP-UP CYCLING (charging already 0 in 194/196 entries from a dock state; median dwell 13s), not noise and not contact bounce. Consequences: `location` flaps AT_DOCK↔ON_LAWN for ~13s while docked, and the undock predicate misses `6→2→1` departures. See `inventory.yaml` § s2p1. | Debunked by: full-corpus s2p1 transition census over 15 probe logs, 2026-07-16; prior claim archived to OLD/…/inventory-history/properties.md. |
...
- single-map era (pre v1.0.3a9) — two remnants found: `_settings_writes.py`
  map_id fallback (T1-6) and `fetch_wifi_map` legacy path (T1-4); both removed
...
- LOCN endpoint era — refresher dead; annotation remnant (T1-7/D18) fixed in
  P1 (refactor-v2/p1-dead-code, 2026-07-02); `fetch_locn` removed in the same
  pass, no longer kept-unscheduled (T1-11).
- `_poll_slow_properties` era — code dead since 2026-05-26 (d7f06555); the
