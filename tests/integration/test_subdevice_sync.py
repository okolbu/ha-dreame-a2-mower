...
def test_sync_tolerates_3_tuple_identifiers_from_other_integrations(
    ...
    with patch.object(coord, "_get_device_registry") as mock_reg:
        ...
        ours = MagicMock()
        ours.identifiers = {(DOMAIN, "G2408000TESTSN0000_map_99")}  # not in wanted
        ours.id = "dev_ours"
...
def test_sync_does_not_prune_when_maps_empty(coordinator_with_two_maps):
    ...
    with patch.object(coord, "_get_device_registry") as mock_reg:
        registry = MagicMock()
        existing = MagicMock()
        existing.identifiers = {(DOMAIN, "G2408000TESTSN0000_map_0")}
