...
def _make_coord(rows):
    """Build a bare _WritesMixin instance wired with capture hooks.

    Returns (coord, captured, write_row, read_rows). The test monkeypatches
    the module-level write_schedule_row / read_schedule_rows with these.
    """
    c = _WritesMixin()
